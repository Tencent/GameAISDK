...
int CPOPUIReg::Predict(const cv::Mat &frame, std::vector<tagPOPUIParam> &stParam,
    cv::Rect &dstRect) {
    float  fVal = 0.0;
    float  fTotal = 1.0f;
    int    nIndex = -1;
...
bool CPOPUIReg::FillPOPUIParam(const UIStateArray &oVecCfg,
    std::vector<tagPOPUIParam> *pstVecParam) {
...
bool CPOPUIReg::IsFrameSame(const cv::Mat &frame) {
    ...
    for (int i = 0; i < blockNumY; i++) {
        for (int j = 0; j < blockNumX; j++) {
            ...
            cv::Rect dstBlkRC = cv::Rect(x, y,
                GAME_UI_DIALOG_BLOCK_WIDTH, GAME_UI_DIALOG_BLOCK_HEIGHT);
