...
#ifndef COMMON_UI_REG_H_
#define COMMON_UI_REG_H_
#include <vector>
...
private:
     ...
     * @return -1 表示失败，否则返回匹配到的UI的ID
     */
    int Predict(const cv::Mat &frame, const std::vector<tagCommonRegParam> &stParam, cv::Rect &dstRect);
     ...
     * @return UI的ID
     */
    // int FindTmplMatchIndex(const std::vector<bool> &bTmplVecMatch, const std::vector<tagCommonRegParam> &stParam);
