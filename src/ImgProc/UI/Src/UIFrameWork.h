...
#ifndef GAME_AI_SDK_IMGPROC_UI_UIFRAMEWORK_H_
#define GAME_AI_SDK_IMGPROC_UI_UIFRAMEWORK_H_
...
class CUIFrameWork {
  public:
    ...
    /*!
      * @brief 初始化
      * @param[in] pszRootPath: 游戏配置(cfg|data)所在的目录绝对路径（默认路径）
      * @param[in] pszUserPath: 用户指定的路径，除了cfg/platform不会替换，其他的配置（cfg｜data）路径配置均会替换
      * @return true表示成功，false表示失败
    */
    bool Initialize(const char *pszRootPath, const char *pszUserPath);
    /*!
      * @brief run
      * return 0表示成功，-1表示失败
    */
  ...
  private:
    ...
    */
    ...
    /*!
      ...
      * @return true表示成功，false表示失败
    */
    bool InitMsgMgr(const char* pszUserCfgPath);
    ...
    */
    ...
    /*!
      ...
      * @return true or false
    */
    bool LoadPlatfromConfig(const char* pszUserCfgPath);
...
#endif  // GAME_AI_SDK_IMGPROC_UI_UIFRAMEWORK_H_
