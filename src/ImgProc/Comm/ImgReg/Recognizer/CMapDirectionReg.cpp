...
int CMapDirectionRegColorDet::Predict(const cv::Mat &oSrcImg, tagMapDirectionRegResult &stResult)
{
    ...
    cv::Mat structureElement = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(m_nDilateSize, m_nDilateSize));
    ...
    structureElement = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(m_nErodeSize, m_nErodeSize));
...
int CMapDirectionRegColorDet::FillColorDetParam(const tagMapDirectionRegParam &stParam, CColorDetParam &oParam)
{
...
int CMapDirectionReg::Initialize(IRegParam *pParam)
{
    ...
    for (int i = 0; i < static_cast<int>(m_oVecParams.size()); i++)
    {
        ...
        if (1 != nState)
        {
            LOGE("task ID %d: CMapDirectionRegColorDet initialization failed, please check", m_nTaskID);
            oMethod.Release();
            return nState;
        }
