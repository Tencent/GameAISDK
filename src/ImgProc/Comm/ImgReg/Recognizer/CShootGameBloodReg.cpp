...
int CShootGameBloodRegMethod::Initialize(const int nTaskID, const tagShootGameBloodRegParam *pParam)
{
    // check task ID
    if (nTaskID < 0)
    {
    ...
    if (1 != nState)
    {
        LOGE("task ID %d: CShootGameBloodRegMethod -- CGradMatch fill param failed, please check", m_nTaskID);
        return nState;
    }
    ...
    if (1 != nState)
    {
        LOGE("task ID %d: CShootGameBloodRegMethod -- CGradMatch initialization failed, please check", m_nTaskID);
    ...
    if (1 != nState)
    {
        LOGE("task ID %d: CShootGameBloodRegMethod -- CColorDet fill param failed, please check", m_nTaskID);
        return nState;
    }
    ...
    if (1 != nState)
    {
        LOGE("task ID %d: CShootGameBloodRegMethod -- CColorDet initialization failed, please check", m_nTaskID);
        m_oColorDet.Release();
        return nState;
    }
...
int CShootGameBloodRegMethod::Predict(const cv::Mat &oSrcImg, tagShootGameBloodRegResult &stResult)
{
    // check source image
    if (oSrcImg.empty())
    {
        LOGE("task ID %d: CShootGameBloodRegMethod -- source image is invalid, please check", m_nTaskID);
        return -1;
    }
    ...
    if (1 != nState)
    {
        LOGE("task ID %d: CShootGameBloodRegMethod -- ROI rectangle is invalid, please check", m_nTaskID);
        return nState;
    }
    ...
    if (1 != nState)
    {
        LOGE("task ID %d: CShootGameBloodRegMethod -- CGradMatch predict failed, please check", m_nTaskID);
        stResult.nState = 0;
    ...
    if (1 != nState)
    {
        LOGE("task ID %d: CShootGameBloodRegMethod -- CColorDet predict failed, please check", m_nTaskID);
        stResult.nState = 0;
        stResult.oROI   = m_oROI;
        return nState;
    }
    ...
    stResult.nState           = 1;
    stResult.oROI             = m_oROI;
    stResult.stBlood.fPercent = static_cast<float>(nMaxX - nMinX) / static_cast<float>(m_nBloodLength);
    stResult.stBlood.oRect    = oBloodRect;
    stResult.stBlood.fScore   = oObjDetResult.m_oVecBBoxes[0].fScore;
...
int CShootGameBloodRegMethod::FillGradMatchParam(const tagShootGameBloodRegParam &stParam, CGradMatchParam &oParam)
{
...
int CShootGameBloodRegMethod::FillColorDetParam(const tagShootGameBloodRegParam &stParam, CColorDetParam &oParam)
{
    oParam.m_nTaskID      = m_nTaskID;
...
int CShootGameBloodReg::Initialize(IRegParam *pParam)
{
    ...
    {
        LOGE("task ID %d: CShootGameBloodReg -- param vector is empty, please check", pP->m_nTaskID);
        return -1;
    }
    ...
    for (int i = 0; i < static_cast<int>(m_oVecParams.size()); i++)
    {
        ...
        if (1 != nState)
        {
            LOGE("task ID %d: CShootGameBloodReg -- CShootGameBloodRegMethod initialization failed, please check",
                m_nTaskID);
            oMethod.Release();
            return nState;
        }
    ...
    LOGD("task ID %d: CShootGameBloodReg -- CShootGameBloodReg initialization successful", m_nTaskID);
    return 1;
}
...
int CShootGameBloodReg::Predict(const tagRegData &stData, IRegResult *pResult)
{
    ...
    if (NULL == pResult)
    {
        LOGE("task ID %d: CShootGameBloodReg -- IRegResult pointer is NULL, please check", m_nTaskID);
        return -1;
    }
    ...
    if (NULL == pR)
    {
        LOGE("task ID %d: CShootGameBloodReg -- CShootGameBloodRegResult pointer is NULL, please check", m_nTaskID);
        return -1;
    }
    ...
    {
        LOGE("task ID %d: CShootGameBloodReg -- frame index %d is invalid, please check", m_nTaskID, stData.nFrameIdx);
        return -1;
    }
    ...
    for (int i = 0; i < static_cast<int>(m_oVecMethods.size()); i++)
    {
        ...
        if (1 != nState)
        {
            LOGE("task ID %d: CShootGameBloodReg -- CShootGameBloodRegMethod predict failed, please check", m_nTaskID);
            return nState;
        }
    }
...
int CShootGameBloodReg::Release()
{
    ...
    for (int i = 0; i < static_cast<int>(m_oVecMethods.size()); i++)
    {
        ...
        if (1 != nState)
        {
            LOGE("task ID %d: CShootGameBloodReg -- CShootGameBloodRegMethod release failed, please check", m_nTaskID);
            return nState;
        }
    }
