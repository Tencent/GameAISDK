...
int CFixBloodRegColorDet::Initialize(const int nTaskID, const tagFixBloodRegParam &stParam) {
    ...
    if (1 != nState) {
        LOGE("task ID %d: CFixBloodRegColorDet -- fill param failed, please check", m_nTaskID);
        return nState;
    }
    ...
    if (1 != nState) {
        LOGE("task ID %d: CFixBloodRegColorDet -- initialization failed, please check", m_nTaskID);
        m_oMethod.Release();
        return nState;
    }
...
int CFixBloodRegColorDet::Predict(const cv::Mat &oSrcImg, tagFixBloodRegResult &stResult) {
    // check parameters
    if (oSrcImg.empty()) {
        LOGE("task ID %d: CFixBloodRegColorDet -- source image is invalid, please check",
            m_nTaskID);
        return -1;
    }
    ...
    if (1 != nState) {
        LOGE("task ID %d: CFixBloodRegColorDet -- ROI rectangle is invalid, please check",
            m_nTaskID);
        return nState;
    }
    ...
    if (1 != nState) {
        LOGE("task ID %d: CFixBloodRegColorDet -- CColorDet predict failed, please check",
            m_nTaskID);
        return nState;
    }
    ...
    findContours(oResult.m_oDstImg, oVecContours, oVecHierarchies,
    ...
    stResult.nState = 1;
    stResult.fPercent = round(((MAX(oRect.size.width, oRect.size.height) / m_nBloodLength)
        * 100.0) / 20.0) * 20.0;
    stResult.oROI = m_oROI;
...
int CFixBloodRegColorDet::FillColorDetParam(const tagFixBloodRegParam &stParam,
    CColorDetParam &oParam) {
...
int CFixBloodReg::Initialize(IRegParam *pParam) {
    ...
    for (int i = 0; i < static_cast<int>(m_oVecParams.size()); i++) {
        ...
        if (1 != nState) {
            LOGE("task ID %d: CFixBloodReg -- initialization failed, please check", m_nTaskID);
            oMethod.Release();
            return nState;
        }
...
int CFixBloodReg::Predict(const tagRegData &stData, IRegResult *pResult) {
    ...
    if (stData.nFrameIdx < 0) {
        LOGE("task ID %d: CFixBloodReg -- frame index %d is invalid, please check",
    ...
    if (NULL == pR) {
        LOGE("task ID %d: CFixBloodReg -- CFixBloodRegResult pointer is NULL, please check",
            m_nTaskID);
        return -1;
    }
    ...
    for (int i = 0; i < static_cast<int>(m_oVecMethods.size()); i++) {
        ...
        if (1 != nState) {
            LOGE("task ID %d: CFixBloodReg -- CFixBloodRegColorDet predict failed, please check",
                m_nTaskID);
            return nState;
        }
    }
...
int CFixBloodReg::Release() {
    ...
    for (int i = 0; i < static_cast<int>(m_oVecMethods.size()); i++) {
        ...
        if (1 != nState) {
            LOGE("task ID %d: CFixBloodReg -- CFixBloodRegColorDet release failed, please check",
                m_nTaskID);
            return nState;
        }
    }
