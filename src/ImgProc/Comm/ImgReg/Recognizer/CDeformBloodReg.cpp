...
int CDeformBloodRegColorDet::Initialize(const int nTaskID, const tagDeformBloodRegParam &stParam) {
    ...
    if (1 != nState) {
        LOGE("task ID %d: CDeformBloodRegColorDet -- CYOLOAPI fill param failed, please check",
            m_nTaskID);
        return nState;
    }
    ...
    if (1 != nState) {
        m_oYOLOAPI.Release();
        LOGE("task ID %d: CDeformBloodRegColorDet --initialization failed, please check",
            m_nTaskID);
        return nState;
    }
    ...
    if (1 != nState) {
        LOGE("task ID %d: CDeformBloodRegColorDet -- fill param failed, please check",
            m_nTaskID);
        return nState;
    }
...
int CDeformBloodRegColorDet::Predict(const cv::Mat &oSrcImg, tagDeformBloodRegResult &stResult) {
    if (oSrcImg.empty()) {
        LOGE("task ID %d: CDeformBloodRegColorDet -- source image is invalid, please check",
            m_nTaskID);
        return -1;
    }
    ...
    if (1 != nState) {
        LOGE("task ID %d: CDeformBloodRegColorDet -- CYOLOAPI predict failed, please check",
            m_nTaskID);
        stResult.nState = 0;
    ...
    for (int i = 0; i < static_cast<int>(oYOLOAPIResult.m_oVecBBoxes.size()); i++) {
        ...
        if (1 != nState) {
            LOGE("task ID %d: CDeformBloodRegColorDet -- CColorDet predict failed, please check",
                m_nTaskID);
            stResult.nState = 0;
            stResult.nBloodNum = 0;
            stResult.oROI = m_oROI;
    ...
    for (int i = 0; i < static_cast<int>(oVecBloods.size()); i++) {
        if (i < MAX_BLOOD_SIZE) {
            ...
            LOGD("task ID %d: CDeformBloodRegColorDet -- blood number is more than max %d",
...
int CDeformBloodRegColorDet::FillColorDetParam(const tagDeformBloodRegParam &stParam,
...
int CDeformBloodRegColorDet::FillYOLOAPIParam(const tagDeformBloodRegParam &stElement,
...
int CDeformBloodReg::Initialize(IRegParam *pParam) {
    ...
    for (int i = 0; i < static_cast<int>(m_oVecParams.size()); i++) {
        ...
        if (1 != nState) {
            LOGE("task ID %d: CDeformBloodReg -- initialization failed, please check",
                m_nTaskID);
            oMethod.Release();
            return nState;
        }
...
int CDeformBloodReg::Predict(const tagRegData &stData, IRegResult *pResult) {
    ...
    if (stData.nFrameIdx < 0) {
        LOGE("task ID %d: CDeformBloodReg -- frame index %d is invalid, please check",
    ...
    if (NULL == pR) {
        LOGE("task ID %d: CDeformBloodReg -- CDeformBloodRegResult pointer is NULL, please check",
            m_nTaskID);
        return -1;
    }
    ...
    for (int i = 0; i < static_cast<int>(m_oVecMethods.size()); i++) {
        ...
        if (1 != nState) {
            LOGE("task ID %d: CDeformBloodReg -- predict failed, please check", m_nTaskID);
            return nState;
        }
    }
...
int CDeformBloodReg::Release() {
    ...
    for (int i = 0; i < static_cast<int>(m_oVecMethods.size()); i++) {
        ...
        if (1 != nState) {
            LOGE("task ID %d: CDeformBloodReg -- release failed, please check", m_nTaskID);
            return nState;
        }
    }
