...
int CMultColorVarRegCalculate::Initialize(const int nTaskID, const tagMultColorVarRegElement &stParam)
{
    if (nTaskID < 0)
    {
    ...
    {
        m_oTmplImg[i]  = cv::imread(stParam.strImageFilePath + '/' + stParam.szDirectionNames[i] + ".png", 1);
        m_oTmplMask[i] = cv::imread(stParam.strImageFilePath + '/' + stParam.szDirectionNames[i] + "Mask.png", 0);
...
int CMultColorVarRegCalculate::Predict(const cv::Mat &oSrcImg, const int nFrameIdx, tagMultColorVarRegResult &stResult)
{
    if (oSrcImg.empty())
    {
...
int CMultColorVarReg::Initialize(IRegParam *pParam)
{
    ...
    for (int i = 0; i < static_cast<int>(m_oVecParams.size()); i++)
    {
        ...
        if (1 != nState)
        {
            LOGE("task ID %d: CMultColorVarRegCalculate initialization failed, please check", m_nTaskID);
            oMethod.Release();
            return nState;
        }
