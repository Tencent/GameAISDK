...
int CShootGameHurtRegMethod::Initialize(const int nTaskID, const tagShootGameHurtRegParam &stParam)
{
    if (nTaskID < 0)
    {
...
int CShootGameHurtRegMethod::Predict(const cv::Mat &oSrcImg, tagShootGameHurtRegResult &stResult)
{
    if (oSrcImg.empty())
    {
        LOGE("task ID %d: CShootGameHurtRegMethod -- source image is invalid, please check", m_nTaskID);
        return -1;
    }
    ...
    if (1 != nState)
    {
        LOGE("task ID %d: CShootGameHurtRegMethod -- ROI rectangle is invalid, please check", m_nTaskID);
        return nState;
    }
    ...
    int nSum  = cv::countNonZero(oLeftImg) + cv::countNonZero(oRightImg) + cv::countNonZero(oBottomImg);
...
int CShootGameHurtReg::Initialize(IRegParam *pParam)
{
    ...
    for (int i = 0; i < static_cast<int>(m_oVecParams.size()); i++)
    {
        ...
        if (1 != nState)
        {
            LOGE("task ID %d: CShootGameHurtReg -- CShootGameHurtRegMethod initialization failed, please check",
                m_nTaskID);
            oMethod.Release();
            return nState;
        }
    ...
    LOGD("task ID %d: CShootGameHurtReg -- CShootGameHurtReg initialization successful", m_nTaskID);
    return 1;
}
...
int CShootGameHurtReg::Predict(const tagRegData &stData, IRegResult *pResult)
{
    ...
    if (NULL == pResult)
    {
        LOGE("task ID %d: CShootGameHurtReg -- IRegResult pointer is NULL, please check", m_nTaskID);
        return -1;
    }
    ...
    if (NULL == pR)
    {
        LOGE("task ID %d: CShootGameHurtReg -- CShootGameHurtRegResult pointer is NULL, please check", m_nTaskID);
        return -1;
    }
    ...
    {
        LOGE("task ID %d: CShootGameHurtReg -- frame index %d is invalid, please check", m_nTaskID, stData.nFrameIdx);
        return -1;
    }
    ...
    for (int i = 0; i < static_cast<int>(m_oVecMethods.size()); i++)
    {
        ...
        if (1 != nState)
        {
            LOGE("task ID %d: CShootGameHurtReg -- CShootGameHurtRegMethod predict failed, please check", m_nTaskID);
            return nState;
        }
    }
...
int CShootGameHurtReg::Release()
{
    ...
    for (int i = 0; i < static_cast<int>(m_oVecMethods.size()); i++)
    {
        ...
        if (1 != nState)
        {
            LOGE("task ID %d: CShootGameHurtReg -- CShootGameHurtRegMethod release failed, please check", m_nTaskID);
            return nState;
        }
    }
