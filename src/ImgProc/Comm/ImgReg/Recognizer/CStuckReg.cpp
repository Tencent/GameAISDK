...
int CStuckRegTmplMatch::Predict(const cv::Mat &oSrcImg, const int nFrameIdx,
    tagStuckRegResult &stResult) {
    // check source image
    if (oSrcImg.empty()) {
...
int CStuckReg::Initialize(IRegParam *pParam) {
    ...
    for (int i = 0; i < static_cast<int>(m_oVecParams.size()); i++) {
        ...
        if (1 != nState) {
            LOGE("task ID %d: CStuckReg -- CStuckRegTmplMatch initialization failed, please check",
                m_nTaskID);
            oMethod.Release();
            return nState;
        }
...
int CStuckReg::Predict(const tagRegData &stData, IRegResult *pResult) {
    ...
    if (stData.nFrameIdx < 0) {
        LOGE("task ID %d: CStuckReg -- frame index %d is invalid, please check",
    ...
    for (int i = 0; i < static_cast<int>(m_oVecMethods.size()); i++) {
        ...
        if (1 != nState) {
            LOGE("task ID %d: CStuckReg -- CStuckRegTmplMatch predict failed, please check",
                m_nTaskID);
            return nState;
        }
    }
...
int CStuckReg::Release() {
    ...
    for (int i = 0; i < static_cast<int>(m_oVecMethods.size()); i++) {
        ...
        if (1 != nState) {
            LOGE("task ID %d: CStuckReg -- CStuckRegTmplMatch release failed, please check",
                m_nTaskID);
            return nState;
        }
    }
