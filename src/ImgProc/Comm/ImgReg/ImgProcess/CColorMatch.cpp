...
int CColorMatch::MatchTemplate(const cv::Mat &oSrcImg, const std::vector<tagTmpl> &oVecTmpls,
    ...
    int nMatchedCount = oVecBBoxes.size();

    if (nMatchedCount > 0) {
        LOGI("CColorMatch::MatchTemplate, TaskId:%d, MaxScore:%.2f(tpl_idx:%d), MatchedCount:%d",
            m_nTaskID, max_score, max_score_index, nMatchedCount);
    }
