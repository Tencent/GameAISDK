...
int CColorDet::Predict(IImgProcData *pData, IImgProcResult *pResult) {
    ...
    pR->m_oDstImg = cv::Mat::zeros(pD->m_oSrcImg.rows, pD->m_oSrcImg.cols,
...
int CColorDet::Detect(const cv::Mat &oSrcImg, cv::Mat &oDstImg,
    std::vector<cv::Point> &oVecPoints) {
    oVecPoints.clear();
    ...
    if (m_nFilterSize > 0) {
        cv::Mat element = cv::getStructuringElement(cv::MORPH_RECT,
            cv::Size(m_nFilterSize, m_nFilterSize));
            ...
            for (int r = y; r < nRows; r += PIX_DET_STRIDE) {
                ...
                for (int c = x; c < nCols; c += PIX_DET_STRIDE) {
                    ...
                    if (nCount > m_nMaxPointNum) {
                        LOGD("task ID %d: the number of color points is larger than max %d",
