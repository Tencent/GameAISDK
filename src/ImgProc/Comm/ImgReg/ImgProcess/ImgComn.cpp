    ...
    std::vector<tagTmpl> &oVecDstTmpls)
{
    ...
    for (int i = 0; i < static_cast<int>(oVecSrcTmpls.size()); i++)
    {
        ...
        if (stSrcTmpl.oTmplImg.empty())
        {
            ...
            if (oTmplImg.empty())
            {
                LOGE("task ID %d: cannot read template image %s, please check", nTaskID, stSrcTmpl.strTmplPath.c_str());
                return -1;
            }
        ...
        if (!stSrcTmpl.strMaskPath.empty())
        {
            ...
            if (oMaskImg.empty())
            {
                LOGE("task ID %d: cannot read mask image : %s, please check", nTaskID, stSrcTmpl.strMaskPath.c_str());
                return -1;
            }
        ...
        for (int j = 0; j < static_cast<int>(oVecScales.size()); j++)
        {
            ...
            {
                LOGW("task ID %d: template size (cols: %d, rows: %d) is too small in scale %f", nTaskID,
                     nCols, nRows, fScale);
                continue;
            }
...
int CheckROI(const int nTaskID, const cv::Mat &oSrcImg, cv::Rect &oROI)
{
    ...
    {
        LOGE("task ID %d: ROI x %d is larger than image cols %d, please check", nTaskID, oROI.x, oSrcImg.cols);
        return -1;
    }
    ...
    {
        LOGE("task ID %d: ROI y %d is larger than image rows %d, please check", nTaskID, oROI.y, oSrcImg.rows);
        return -1;
    }
...
int DrawBBox(const cv::Mat &oSrcImg, const std::vector<tagBBox> &oVecBBoxes, const int nWaiteTime, const int nClassID)
{
            ...
            std::vector<std::string> *pVecPaths, std::vector<std::string> *pVecNames)
{
    ...
    if (hFile == -1)
    {
        LOGE("task ID %d: template directory %s is invalid, please check", nTaskID, strPath.c_str());
        _findclose(hFile);
        return -1;
    }
    ...
    std::vector<std::string> *pVecPaths, std::vector<std::string> *pVecNames)
{
    ...
    {
        LOGE("task ID %d: template directory %s is invalid, please check", nTaskID, strDirectory.c_str());
        return -1;
    }
            ...
            cv::Mat &oDstImg)
{
    ...
    cv::Mat oValueImg(oSrcImg.rows, oSrcImg.cols, CV_8UC3, cv::Scalar(nMaskValue, nMaskValue, nMaskValue));
    cv::Mat oMaskRegion;
