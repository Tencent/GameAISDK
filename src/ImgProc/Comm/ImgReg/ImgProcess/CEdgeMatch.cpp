...
int CEdgeMatch::ParseParam(const CEdgeMatchParam *pParam)
{
    ...
    for (int i = 0; i < static_cast<int>(m_oVecTmpls.size()); i++)
    {
        ...
        if (1 != nState)
        {
            LOGE("task ID %d: CEdgeMatch -- extract edge in template image failed, please check", m_nTaskID);
            return nState;
        }
...
int CEdgeMatch::ExtractEdge(const cv::Mat &oSrcImg, cv::Mat &oDstImg)
{
    ...
    cv::adaptiveThreshold(oGrayImg, oDstImg, 255.0, cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY, 7, 5);
    ...
    std::vector<tagBBox> &oVecBBoxes)
{
    ...
    if (1 != nState)
    {
        LOGE("task ID %d: CEdgeMatch -- extract edge in source image failed, please check", m_nTaskID);
        return nState;
    }
    ...
    for (int i = 0; i < static_cast<int>(oVecTmpls.size()); i++)
    {
        ...
        {
            LOGD("task ID %d: CEdgeMatch -- edge image is smaller than template image in scale %f", m_nTaskID, fScale);
            continue;
        }
