...
void CGradMatch::QuantAngle(const cv::Mat &oMagnitude, cv::Mat &oAngle, cv::Mat &oOrientation,
    int nWeakThreshold) {
...
void CGradMatch::SpreadOrientation(const cv::Mat &oOrientation, cv::Mat &oBinaryMat, int nT) {
    ...
    for (int r = 0; r < nT; r++) {
        ...
        for (int c = 0; c < nT; c++) {
            // orientation(r, c)
            const uchar *pOrientation = oOrientation.data +
                r * oOrientation.step + c * oOrientation.elemSize();
...
void CGradMatch::ComputeResponseMap(const cv::Mat &oBinaryMat,
...
void CGradMatch::ComputeSimilarity(const std::vector<cv::Mat> &oVecLinearizedMap,
    ...
    for (int i = 0; i < static_cast<int>(stGradTemplate.oVecGradFeature.size()); i++) {
        ...
        if (stGradFeature.nX < 0 || stGradFeature.nX >= nCols || stGradFeature.nY < 0
            || stGradFeature.nY >= nRows)
            continue;
...
int CGradMatch::MatchTemplate(const std::vector<cv::Mat> &oVecLinearizedMap,
    ...
    for (int i = 0; i < static_cast<int>(oVecGradTempalte.size()); i++) {
        ...
        cv::Mat         oSimilarity
            = cv::Mat::zeros(nRows / nT, nCols / nT, cv::DataType<int16_t>::type);
            ...
            for (int c = 0; c < oSimilarity.cols; c++) {
                ...
                if (nRawSimilarity > nRawThreshold) {
                    ...
                    if (nSimilarity / 100.f >= stGradTemplate.fThreshold) {
                        if (nX + stGradTemplate.nWidth <= nCols &&
                            nY + stGradTemplate.nHeight <= nRows) {
                            ...
                            stDetectBBox.oRect = cv::Rect(nX, nY, stGradTemplate.nWidth,
                                stGradTemplate.nHeight);
...
int CGradMatch::MakeTemplate(const std::vector<tagTmpl> &oVecTmpls,
...
int CGradMatch::ExtractFeature(const cv::Mat &oMagnitude, const cv::Mat &oOrientation,
    ...
    if (nFeatureNum <= 0) {
        LOGE("task ID %d: CGradMatch -- feature number %d is invalid, please check",
            m_nTaskID, nFeatureNum);
        return -1;
    }
...
int CGradMatch::Detect(const cv::Mat &oSrcImg, const std::vector<tagGradTemplate> &oVecGradTmpls,
    ...
    if (oVecGradTmpls.empty()) {
        LOGE("task ID %d: CGradMatch -- gradient template vector is empty, please check",
            m_nTaskID);
        return -1;
    }
    ...
    for (int i = 0; i < 8; i++) {
        cv::Mat oLinearizedMap = cv::Mat::zeros(nT * nT, (nRows - nRows % nT) *
            (nCols - nCols % nT) / (nT * nT), cv::DataType<uchar>::type);
