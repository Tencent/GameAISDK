...
int CORBMatch::ParseParam(const CORBMatchParam *pParam) {
    ...
    // set ORB
    m_oORB = cv::ORB::create(pParam->m_nFeatureNum, pParam->m_fScaleFactor, pParam->m_nLevel,
        pParam->m_nEdgeThreshold);
...
int CORBMatch::MatchTemplate(const cv::Mat &oSrcImg, const std::vector<tagTmpl> &oVecTmpls,
    ...
    for (int i = 0; i < static_cast<int>(oVecTmpls.size()); i++) {
        ...
        float fScale = (static_cast<float>(stTmpl.oTmplImg.rows)
            ...
            static_cast<float>(stTmpl.oTmplImg.cols)
        ...
        // {
        // LOGD("task ID %d: CORBMatch -- source image is smaller than template image in scale %f",
        // m_nTaskID, fScale);
        //    continue;
        // }
        ...
        if (oVecTmplPoints.empty()) {
            LOGI("task ID %d: CORBMatch -- cannot extract ORB feature in template image",
                m_nTaskID);
            continue;
        }
        ...
        float fScore = nMatchNum / MIN(static_cast<float>(oSrcFeature.rows),
            static_cast<float>(oTmplFeature.rows));
        if (fScore > 1.0) {
            fScore = 1.0f;
        }
