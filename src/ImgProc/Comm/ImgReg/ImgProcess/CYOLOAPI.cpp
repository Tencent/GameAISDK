...
int CYOLO::Predict(const cv::Mat &oSrcImg, std::vector<tagBBox> &oVecBBoxes)
{
    ...
    detection *pDetResults = get_network_boxes(&stNet, oSrcImg.cols, oSrcImg.rows, m_fThreshold, .5, 0, 1, &nBoxes, 0);
    ...
    // get detection results
    detection *pDetResults = get_network_boxes(pFreeNet, oSrcImg.cols, oSrcImg.rows, m_fThreshold, .5, 0, 1, &nBoxes);
...
int CYOLOAPI::Initialize(const CYOLOAPIParam &oParam)
{
    ...
    {
        LOGE("task ID %d: mask value %d is invalid, please check", oParam.m_nTaskID, oParam.m_nMaskValue);
        return -1;
    }
    ...
    {
        LOGE("task ID %d: threshold %f is invalid, please check", oParam.m_nTaskID, oParam.m_fThreshold);
        return -1;
    }
    ...
    if (!oParam.m_strMaskPath.empty())
    {
        ...
        if (m_oMask.empty())
        {
            LOGE("task ID %d: cannot read mask image : %s, please check", m_nTaskID, oParam.m_strMaskPath.c_str());
            return -1;
        }
