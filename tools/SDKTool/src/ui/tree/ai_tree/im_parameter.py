...
from ...utils import valid_number_value, tool_to_sdk_path, sdk_to_tool_path

logger = logging.getLogger("sdktool")


class IMParameter(object):
    def __init__(self):
    ...
    def init(self):
        if not self.__parameter:
            return self._ai_mgr.load_learning_config(AIAlgorithmType.IM)
        return True
    ...
    def __get_params(self):
        if self._ai_parameter_inst is None:
            self._ai_parameter_inst = self._ai_mgr.get_ai_parameter(AIAlgorithmType.IM)
