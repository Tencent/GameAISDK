...
class AgentAPIMgr(object):
    ...
    def Initialize(self, confFile, referFile=None, index=1, selfAddr=None, cfgPath='../cfg/platform/bus.ini'):
        """
    ...
    def SendUISrcImage(self, srcImgDict):
        return self.__msgMgr.ProcUISrcImgMsg(srcImgDict)

    def RecvUIResult(self):
        return self.__msgMgr.RecvUI()
    ...
    def _Check(self, gameRets):
        ...
        if self.__groupDict['groupID'] != groupID:
            LOG.error('recv group ID [{0}] in reg results not match '
                      'self group ID [{1}], please check'.format(groupID, self.__groupDict['groupID']))
    ...
    def _ProcSendGroupID(self, cmdValue):
        groupDict = None
        groupList = self.__gameTaskConfig.get('alltask') or self.__gameTaskConfig.get('allTask') or []
