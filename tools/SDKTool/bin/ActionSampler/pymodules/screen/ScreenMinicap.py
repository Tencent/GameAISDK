...
class ScreenMinicap(MinicapStream, RotationWatcher):
    def __init__(self, device, isShow=False):
        super(self.__class__, self).__init__(device, isShow)

    def Initialize(self, capHeight = 720, capWidth = 1280):
        self.open_rotation_watcher(on_rotation_change=lambda v: self.OpenMinicapStream(capHeight, capWidth))
