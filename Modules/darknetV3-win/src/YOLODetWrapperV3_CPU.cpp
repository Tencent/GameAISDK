...
class YOLODetWrapperV3_CPU
{
public:
	YOLODetWrapperV3_CPU();
	~YOLODetWrapperV3_CPU();
...
YOLODetWrapperV3_CPU::YOLODetWrapperV3_CPU()
{
...
bool YOLODetWrapperV3_CPU::Initialize(char* pszNetFile, char *pszWeightFile)
{
	if (NULL == pszNetFile)
	{
...
boost::python::list  YOLODetWrapperV3_CPU::Detect(int rows, int cols, PyObject* pImageData)
{
...
BOOST_PYTHON_MODULE(YOLODetWrapperV3_CPU)
{
	boost::python::class_<YOLODetWrapperV3_CPU>("YOLODetWrapperV3_CPU")
		.def("Initialize", &YOLODetWrapperV3_CPU::Initialize)
		.def("Detect", &YOLODetWrapperV3_CPU::Detect);
