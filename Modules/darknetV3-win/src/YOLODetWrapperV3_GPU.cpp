...
class YOLODetWrapperV3_GPU
{
public:
	YOLODetWrapperV3_GPU();
	~YOLODetWrapperV3_GPU();
	bool Initialize(char* pszNetFile, char *pszWeightFile);
	boost::python::list  Detect(int rows, int cols, PyObject* imgData);

private:
	CYOLOV3Det m_oDetector;

};


YOLODetWrapperV3_GPU::YOLODetWrapperV3_GPU()
{
...
bool YOLODetWrapperV3_GPU::Initialize(char* pszNetFile, char *pszWeightFile)
{
	if (NULL == pszNetFile)
	{
...
boost::python::list  YOLODetWrapperV3_GPU::Detect(int rows, int cols, PyObject* pImageData)
{
...
BOOST_PYTHON_MODULE(YOLODetWrapperV3_GPU)
{
	boost::python::class_<YOLODetWrapperV3_GPU>("YOLODetWrapperV3_GPU")
		.def("Initialize", &YOLODetWrapperV3_GPU::Initialize)
		.def("Detect", &YOLODetWrapperV3_GPU::Detect);
